...
public:
    ...
    void startGame()
    {
        if (roundNumber < 5 && this->connectedClients.size() >= 2)
        {
    ...
    void buildRanking()
    {
        ...
        for (const auto &entry : this->connectedClients)
        {
            if (connectedClients[entry.first].gameMaster == false)
            {
        ...
        for (const auto &entry : this->connectedClients)
        {
            ...
            if (client.gameMaster == false)
            {
                std::cout << "login: " << client.clientLogin << " punkty: " << client.clientPoints << std::endl;
            }
        }

        // wyslij punkty do gamemastra
        for (const auto &entry : this->connectedClients)
        {
            const auto &client = entry.second;
            if (client.gameMaster == true)
            {
                std::string ranking = "Ranking\t";
                for (const auto &entry1 : this->connectedClients)
                {
                    const auto &client1 = entry1.second;

                    if (client1.gameMaster == false)
                    {
                        ranking = ranking + " login: " + client1.clientLogin + " punkty: " + std::to_string(client1.clientPoints) + "\t";
                    }
                }
                ranking = ranking + "\n";
                ssize_t bytesSent = send(client.clientSocket, ranking.c_str(), ranking.size(), 0);
                if (bytesSent == -1)
                {
                    perror("Error sending ranking to game master");
                }
                std::cout << "wyslano ranking do gamemastera" << ranking << std::endl;
            }
        }
    };
    void endGame()
    {
        ...
        ranking = ranking + "KONIEC GRY!\n";
        // wyslij ranking do wszystkich klientow
        for (const auto &entry : this->connectedClients)
        {
            const auto &client = entry.second;
            ssize_t bytesSent = send(client.clientSocket, ranking.c_str(), ranking.size(), 0);
        ...
        else if (epollEvent.events & EPOLLIN && epollEvent.data.fd != serverSocket)
        {
            ...
            while (true)
            {
                ...
                else if (bytesRead == 0)
                {
                    ...
                    int gameCode = connectedClients[clientSocket].gameCode;
            ...
            if (clientResponse1[0] == 'o')
            {
                ...
                for (int i = 0; i < static_cast<int>(connectedClients.size()); ++i)
                {
                    if (connectedClients[i].gameCode == gameCode && connectedClients[i].gameMaster == true)
                    {
                        ...
                        //       games[connectedClients[i].gameCode].connectedClients[clientSocket1].clientSocket =clientSocket1;
                        break;
                    }
                }
            }
            ...
            //    std::cout << "pytanie: " << question << " odpowiedz: " << answer << std::endl;
                ...
                if (connectedClients[clientSocket].gameMasterQuestionCounter == 5)
                {
                    ...
                    // dodaj gamemastera do gry
                    games[gameCode].connectedClients.insert(std::pair<int, Client>(clientSocket, connectedClients[clientSocket]));
                    // ustaw gamemaster na true
                    games[connectedClients[clientSocket].gameCode].connectedClients[clientSocket].gameMaster = true;

                    games[connectedClients[clientSocket].gameCode].connectedClients[clientSocket].clientSocket = clientSocket;
                }
